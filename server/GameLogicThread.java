...
public class GameLogicThread extends Thread {
	 ...
	 */
	public void collide(String collision) {// TODO

		// once the server has calculated that an item is in the same place on
		// the headpoint of the snake
		// once the server has calculated that a snake has collided headon with
		// another snake
		// once the server has calculated that a snake has hit a body part of
		// another snake.
	 ...
	 */
	public synchronized void updateBoard(Map<Point, String> board) {
		...
		// check for item over head

		for (int i = 0; i < 2; i++) {

			if (snakeGame.getPlayers()[i].getSnake().getSnakeBody().get(0)
					.getLocation() == snakeGame.getGame().getItems()[0]
					.getLocation()) {

				System.out.println("Have found that an item has been eaten");
				collide(new String("HvI"));

			}

		}
		// check for head on head collision
		// check for if a head has hit another snakes body part
